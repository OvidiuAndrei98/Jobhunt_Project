...
import com.lancefy.backend.model.jobs.*;
...
public class Data implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        Job job = new Job("Help with JPA Springboot,Docker and kafka", "web-development", new JobBudget(PaymentType.HOURLY, "$20-$30/hr" ), "Less than 30 hrs/week", LocalDateTime.now(),"I have an springboot application with Service and controllers working and model object has to be mapped to postgreSQL and containerize the application using docker.\n" +
                "\n" +
                "optionally, we can create a kafka consumer-producer in the application", List.of("Docker", "Kafka", "Spring"), "Romania", new JobScope("Large", "Less than 1 month", ExperienceLevel.Intermediate));
        Job job2 = new Job("Javascript Single Spa Expert", "web-development", new JobBudget(PaymentType.HOURLY, "$20-$30/hr" ), "Less than 30 hrs/week",  LocalDateTime.now(),"Looking for an expert with single spa micro frontends to enable our existing web application running behind root config.\n" +
                "\n" +
                "We are looking for someone that can setup the right configs and get everything running in a short amount of time.", List.of("Java", "Docker", "Kafka"),"USA", new JobScope("Large", "Less than 1 month", ExperienceLevel.Intermediate));
        Job job3 = new Job("Bookkeeping for 2 closely-held LLCs", "accounting", new JobBudget(PaymentType.HOURLY, "$20-$30/hr" ), "Less than 30 hrs/week", LocalDateTime.now(),"Need a bookkeeper to update books and transition to QB online.", List.of("Accounting", "Data Entry"),"USA", new JobScope("Large", "Less than 1 month", ExperienceLevel.Intermediate));
